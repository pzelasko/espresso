#!/bin/bash
# Copyright (c) Yiming Wang, Piotr Żelasko
#
...
if [[ $(hostname -f) == *.clsp.jhu.edu ]]; then
  amidir=/export/corpora5/AMI/amicorpus
fi
kaldi_scoring=true
...
if [ ${stage} -le 1 ]; then
  ...
  ## compute global CMVN
  compute-cmvn-stats scp:data/${mic}/${train_set}/feats.scp data/${mic}/${train_set}/cmvn.ark
  ...
  dump.sh --cmd "$train_cmd" --nj 32 --do_delta $do_delta \
    data/${mic}/${train_set}/feats.scp data/${mic}/${train_set}/cmvn.ark exp/dump_feats/train ${train_feat_dir}
  dump.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
    data/${mic}/${valid_set}/feats.scp data/${mic}/${train_set}/cmvn.ark exp/dump_feats/valid ${valid_feat_dir}
  dump.sh --cmd "$train_cmd" --nj 4 --do_delta $do_delta \
    data/${mic}/${test_set}/feats.scp data/${mic}/${train_set}/cmvn.ark exp/dump_feats/test ${test_feat_dir}
fi
...
if [ ${stage} -le 2 ]; then
  ...
  train_text=data/${mic}/$train_set/text
  # AMI doesn't have non lignuistic symbols
  ...
  for dataset in $train_set $valid_set $test_set; do
    text=data/${mic}/$dataset/text
    token_text=data/${mic}/$dataset/token_text
  ...
  if ! $use_wordlm; then
    ...
    #cp $wordlmdatadir/$train_set $wordlmdatadir/train
...
PYTHON=/home/pzelasko/miniconda3/envs/espresso-ami/bin/python
...
if [ ${stage} -le 3 ]; then
  ...
  if ! $use_wordlm; then
    ...
    ${decode_cmd} $lmdatadir/log/preprocess.log \
      $PYTHON ../../fairseq_cli/preprocess.py --task language_modeling_for_asr \
...
if [ ${stage} -le 4 ] && ! $use_wordlm; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu $PYTHON ../../fairseq_cli/train.py $lmdatadir --seed 1 \
    ...
    --distributed-world-size $ngpus --reset-optimizer \
...
if [ ${stage} -le 5 ] && ! $use_wordlm; then
  ...
  for gen_subset in valid test; do
    ...
    $PYTHON ../../fairseq_cli/eval_lm.py $lmdatadir --cpu \
...
if [ ${stage} -le 6 ] && $use_wordlm; then
  ...
  update_freq=$(((2+ngpus-1)/ngpus))
  CUDA_VISIBLE_DEVICES=$free_gpu $PYTHON ../../fairseq_cli/train.py $wordlmdatadir --seed 1 \
...
if [ ${stage} -le 7 ] && $use_wordlm; then
  ...
  for gen_subset in valid test; do
    ...
    $PYTHON ../../fairseq_cli/eval_lm.py $wordlmdatadir --cpu \
...
if [ ${stage} -le 8 ]; then
  ...
  train_text=data/${mic}/$train_set/text
  train_utt2num_frames=data/${mic}/$train_set/utt2num_frames
  valid_feat=$valid_feat_dir/feats.scp
  valid_text=data/${mic}/$valid_set/text
  valid_utt2num_frames=data/${mic}/$valid_set/utt2num_frames
  ...
  for dataset in $valid_set $test_set; do
    ...
    text=data/${mic}/$dataset/text
    utt2num_frames=data/${mic}/$dataset/utt2num_frames
...
if [ ${stage} -le 9 ]; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu $PYTHON ../../fairseq_cli/train.py data --task speech_recognition_espresso --seed 1 \
    --log-interval $((800/ngpus/update_freq)) --log-format simple --print-training-sample-interval $((2000/ngpus/update_freq)) \
    --num-workers 3 --data-buffer-size 0 --max-tokens 44000 --batch-size 64 --curriculum 2 --empty-cache-freq 50 \
    --valid-subset $valid_subset --batch-size-valid 128 --ddp-backend legacy_ddp --update-freq $update_freq \
    --distributed-world-size $ngpus --fp16 \
...
if [ ${stage} -le 10 ]; then
  ...
  for dataset in $test_set; do
    ...
    CUDA_VISIBLE_DEVICES=$(echo $free_gpu | sed 's/,/ /g' | awk '{print $1}') $PYTHON ../../espresso/speech_recognize.py data \
      --task speech_recognition_espresso --max-tokens 20000 --batch-size 16 \
    ...
    if $kaldi_scoring; then
      ...
      local/score_e2e.sh data/$mic/$dataset $decode_dir
